...
public class GuessTheNumberEjbServiceImpl implements GuessTheNumberEjbService {
    ...
    /**
     * return the list of all or the first twelve players from the data base
     * depends on the input, true for the first twelve players data
     * false for all the players data
     */
	@Override
	public List<Player> listTheFirstTwelveOrAllPlayers(boolean isListAll) {
		try {
			TypedQuery<Player> query = entityManager.createQuery("select p from Player p "
					+ "order By p.numberOfGuesses ASC ",Player.class);
			if (!isListAll) {
	...
	/**
	 * inserts the player data to the data base table
	 * if the insert is ok (the name must be unique) then returns the id
	 * and if not (throws sql exception) returns -2 or -1
	 */
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	@Override
...
/**
 * 	generate a game id and save a new game into the
 *  games status hash map with a new number for the player to guess.
 */
	...
	@Override
	/**
	 * the method takes the guessed number and return validation results for the client
	 */
		...
		if (check.getGuessedNumber().length() == 4) {
		   ...
		   if(checkIfTheNumberContainACharacters(check.getGuessedNumber())) {
			  ...
			  if (checkIfTheNumberDoesNotContainAduplicateDigits(check.getGuessedNumber())) {
				...
				String number =GuessTheNumberEjbServiceImpl.gamesStatus.get(check.getGameId()).getNumber();
				
				results.setTheNumberIsGuessed(checkTheDigitsOfTheNumberAndOfTheGuessedNumber(number,check.getGuessedNumber()));
				int numberOfGuesses =GuessTheNumberEjbServiceImpl.gamesStatus.get(check.getGameId()).getNumberOfGuesses();
				results.setNumberOfGuesses(numberOfGuesses+1);
				GuessTheNumberEjbServiceImpl.gamesStatus.get(check.getGameId()).setNumberOfGuesses(numberOfGuesses+1);
	 ...
	 * @param number
	 * @return true if the number does not contain character
	 * 		   false if it does
	 */
	 ...
	 * 
	 * @param number
	 * @return true if the number has different digits
	 * 		   false if he does
	 */
	...
	/**
	 * checks if the digits in the number are equal and if the digits  in the same index or not
	 * @param number
	 * @param guessedNumber
	 * @return a string of zeros and ones that resembles the equal digits,"0" not in the same index ,"1" in the same index
	 */
